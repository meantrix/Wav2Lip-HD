...
def main():
    ...
    if not os.path.exists('temp/result.avi'):
        print("Error: temp/result.avi was not created, something went wrong during video processing.")
    else:
        print("Successfully created temp/result.avi")
    ...
    if args.save_frames and args.save_as_video:
        ...
        command = ['ffmpeg', '-y', '-i', 'temp/gt.avi', '-i', args.audio, '-strict', '-2', '-q:v', '1', args.gt_path]
        subprocess.call(command)

        command = ['ffmpeg', '-y', '-i', 'temp/pred.avi', '-i', args.audio, '-strict', '-2', '-q:v', '1', args.pred_path]
        subprocess.call(command)
